...
class Sale_lib
{
	...
	*/
		...
		if($iva==1 || $iva=="1")
		{
			...
			{
				$price_to_use = $this->_get_price_for_item_in_cart($item);
			    $subtotal+=(((float)($price_to_use*$item['quantity']-$price_to_use*$item['quantity']*$item['discount']/100))+((float)(($price_to_use*$item['quantity']-$price_to_use*$item['quantity']*$item['discount']/100)*0.16)));
			}
		echo "subtotal".$subtotal;
	...
	*/
	...
	function iva_all($percent_discount)
	{
		$items = $this->get_cart();
