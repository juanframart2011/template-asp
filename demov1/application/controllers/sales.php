...
class Sales extends Secure_area
{
	...
	function index()
	{
		...
		//$datos['pedidos']= $this->pedidos->pedidos();
		//$this->load->view('cuentas',$datos);
	}

	function venta()
	{
		$this->load->view("partial/header");
		$this->load->view("sales/venta");
	...
	function iva()
	{
		...
		/*
		if($iva==1){
			$total=(float)$total+(float)($total*(0.16));
		}else{
			$total=$total;
		}
		
		$this->sale_lib->get_total(false,$total);
		
		$discount_all_percent = (int)$this->input->post('discount_all_percent');
		*/
		$this->sale_lib->iva($iva);
		$this->_reload();
	}
	...
	function complete()
	{
		...
		if (empty($this->sale_lib->get_payments()))
		{
			
			echo ("<script>alert('Si el cliente no te dejo Anticipo, Agrega un pago en Efectivo de $0, para poder efectuar la venta.');</script>");
			//redirect('sales');
			header( "refresh:0; url=sales" );
			return false;
		}
		...
		if($customer_id!=-1)
		{
			...
			$data['person_id']=$cust_info->person_id;
			$data['id_venta']=$sale_id;
	...
	function recibo($sale_id)
	{
		...
		$this->load->view("sales/recibo",$data);
		$this->sale_lib->clear_all();
