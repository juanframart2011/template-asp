...
<div id="receipt_wrapper">
	<div id="receipt_header">
		...
		<?php $id_venta= str_replace("Sy ", "", $sale_id); ?>
   ...
   <br />   
	...
	<form method="post" action="index.php/factura/realizar_factura/<?php echo $id_venta; ?>/<?php echo $person_id; ?>">
		<!--
			<button class="btn btn-primary text-white hidden-print" type="submit" >Generar formato factura. </button>-->
		</form>
...
<div class=""  style="float: right; display: none;">
	<input  id="tipopago" type="hidden" value="<?php $splitpayment=explode(':',$payment['payment_type']); echo $splitpayment[0]; ?>">
		<input  id="ganancia" type="hidden" value="<?php echo ( $payment['payment_amount'] ); ?>">
		<input type="hidden" id="resto" name="resto" value="<?php echo ($amount_change); ?>" >
		<div id="sale_id"><?php echo lang('sales_id').":".$sale_id; ?></div>
		<input id="restoid" type="hidden" value="<?php echo $sale_id ?>">
		<div id="employee"><?php echo lang('employees_employee').": ".$employee; ?></div>
		<input type="hidden" id="restocl" value="<?php echo $customer; ?>" ></div>
		<input type="pago" name="pago" value="<?php echo ( $payment['payment_amount'] ); ?>">
<script>
   ...
   $(document).ready(function(){
 			...
 			sale_id2=$('#restoid').val();
 			patron ="MP ";
 			nuevoValor ="";
 			sale_id = sale_id2.replace(patron, nuevoValor);
 			preci = $('#preci').val();
			cliente=$('#restocl').val();
			resto=$('#resto').val()*-1;
			precio = $('#ganancia').val();
	    	...
	    	item_id =  $('#item_id').val();
				tipo= tipop;
			pago=<?php echo ($payment['payment_amount']); ?>;
			var total_final=(parseFloat(total)+parseFloat(resto));

      if(parseFloat(pago)>parseFloat(total_final)){
      	abono=parseFloat(total_final);
      }else{
      	abono=parseFloat(pago);
      }
     
      
      console.log("pago:"+pago+"abono"+abono);

      	$.ajax({
      		url:"<?php echo base_url().'index.php/abonos/crear_abono'; ?>",
      		type:"POST",
      		data:{sale_id:sale_id,tipo_pago:tipo,total_final:total_final,resto:resto,abono:abono,cliente:cliente},
      ...
      /*
           $.ajax({url:"<?php echo base_url().'index.php/welcome/abono'; ?>",type:'POST',data:{sale_id:sale_id,cliente:cliente,resto:resto,precio:precio,tipo:tipo,total:total,total_final:total_final,item_id:item_id},success:function(data){
               ...
               console.log('preci ',preci);
   				console.log('sal_id  '+sale_id);
   				console.log('total  '+(parseFloat(total)+parseFloat(resto)));
          }});
	*/
